...
interface GraficoCruceLineaProps {
  registros: RegistroGenerico[];
  porMotivo: { nombre: string; cantidad: number }[];
  porLinea: { linea: string; cantidad: number }[];
  colNombreFila: string | null;
  colNombreLinea: string | null;
...
export function GraficoCruceLinea({
  registros,
  porMotivo,
  porLinea,
  colNombreFila,
  ...
  const columnas = useMemo(
    () => porLinea.slice(0, MAX_LINEAS).map((l) => l.linea),
    [porLinea]
  );

  const { filas, matrix, maxPorFila, totalesPorFila } = useMemo(() => {
    if (!colNombreFila || !colNombreLinea) {
      return { filas: [], matrix: new Map<string, Map<string, number>>(), maxPorFila: new Map(), totalesPorFila: new Map() };
    }
    const filas = porMotivo.slice(0, limiteFila).map((m) => m.nombre);
    const colSet = new Set(columnas);
    const matrix = new Map<string, Map<string, number>>();
    filas.forEach((m) => matrix.set(m, new Map()));

    registros.forEach((reg) => {
      const filaVal = (reg.valores[colNombreFila] ?? "").trim();
      if (!matrix.has(filaVal)) return;
      const colVal = (reg.valores[colNombreLinea] ?? "").trim();
      if (!colVal || !colSet.has(colVal)) return;
  ...
  }, [registros, porMotivo, columnas, limiteFila, colNombreFila, colNombreLinea]);
  ...
  if (porLinea.length < 2 || porMotivo.length < 2 || !colNombreFila || !colNombreLinea) return null;

  const validLimites = LIMITES_FILA.filter((l) => l <= porMotivo.length);
  const pctTooltip =
  ...
  const labelLinea = colNombreLinea.length > 20 ? colNombreLinea.slice(0, 19) + "…" : colNombreLinea;
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-semibold text-slate-800 dark:text-slate-100">
              {labelFila} por {labelLinea}
            </h3>
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">
              Top {MAX_LINEAS} valores de {labelLinea} · intensidad normalizada por fila
            </p>
          </div>
        </div>
            ...
            <tr className="bg-slate-50 dark:bg-[#0d0f14]/50">
              ...
              {columnas.map((linea) => (
                <th
                  ...
                  title={linea}
                >
                  {linea.length > 6 ? linea.slice(0, 5) + "…" : linea}
                </th>
              ))}
            ...
            {filas.map((fila, rowIdx) => {
              ...
              return (
                <tr
                  key={fila}
                  className={`${
                    rowIdx % 2 === 0 ? "" : "bg-slate-50/50 dark:bg-[#0d0f14]/20"
                ...
                >
                  <td
                    ...
                    style={{ maxWidth: 196 }}
                  >
                    {fila.length > 28 ? fila.slice(0, 27) + "…" : fila}
                  </td>
                  {columnas.map((linea) => {
                    const count = rowMap.get(linea) ?? 0;
                    ...
                    return (
                      <td
                        key={linea}
                        className="text-center py-1.5 border-r border-slate-100 dark:border-[#1f2535]/50 cursor-default select-none"
                        style={{ backgroundColor: bg }}
                        onMouseEnter={(e) =>
                          count > 0 && handleMouseEnter(e, fila, linea, count)
                        }
          ...
          <div className="chart-tooltip" style={{ minWidth: 210 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 16 }}>
                <span style={{ color: "var(--tooltip-muted)" }}>{labelLinea}:</span>
              ...
              >
                <span style={{ color: "var(--tooltip-muted)" }}>% del valor:</span>
                <span style={{ color: "var(--tooltip-text)", fontWeight: 700 }}>
                  {pctTooltip}%
                </span>
              </div>
